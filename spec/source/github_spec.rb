...
describe "CodeInventory::Source::GitHub" do
  ...
  describe ".projects" do
    it "provides a list of projects" do
      stub_request(:get, "https://api.github.com/orgs/GSA/repos?per_page=100").to_return(:status => 200, :body => file_fixture("github_two_repositories_response.json"), :headers => {"Content-Type" => "application/json"})
      stub_request(:get, "https://api.github.com/repos/GSA/ProductOne/contents/.codeinventory.yml").to_return(:status => 200, :body => file_fixture("github_product_one_contents_response.json"), :headers => {"Content-Type" => "application/json"})
